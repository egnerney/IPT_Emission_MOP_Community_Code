...
=====================
Create MP4 Movies from UV Emission Frame Sequences

This script uses FFmpeg to combine the PNG frame sequences generated by
make_emission_movie_frames.py into MP4 video files.

OUTPUT MOVIES:
1. total_uv_emission.mp4      - Total UV emission (550-2100 Å)
2. s2p_emission.mp4           - S++ emission (680 Å)
3. op_emission.mp4            - O+ emission (833 Å)
...
USAGE:
Run this script after make_emission_movie_frames.py has completed:
    python make_movies_ffmpeg.py
...
FRAME_DIRS = {
    'total_uv': FRAME_BASE / "frames_total_uv",
    's2p': FRAME_BASE / "frames_s2p_uv",
    'op': FRAME_BASE / "frames_uv",
    'stacked': FRAME_BASE / "frames_stacked_uv"
}

# Output movie names
OUTPUT_MOVIES = {
    'total_uv': "total_uv_emission.mp4",
    's2p': "s2p_emission_uv.mp4",
    'op': "op_emission_uv.mp4",
    'stacked': "stacked_emission_uv.mp4"
}

# Movie descriptions for logging
MOVIE_DESCRIPTIONS = {
    'total_uv': "Total UV Emission (550-2100 Å)",
    's2p': "S++ Emission (680 Å)",
    'op': "O+ Emission (833 Å)",
...
def main():
    ...
    print("=" * 70, flush=True)
    print("Io Plasma Torus UV Emission Movie Creator", flush=True)
    print("=" * 70, flush=True)
    print(flush=True)
    ...
    if not all_dirs_exist:
        ...
        print("Please run make_emission_movie_frames.py first.", flush=True)
        return
    ...
    for name in ['total_uv', 's2p', 'op', 'stacked']:

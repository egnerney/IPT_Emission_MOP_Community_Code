...
FUNCTION IPT_INTEGRATE_SPECIES_EMISSION_SINGLE, plasma_model, tables_single, $
  ...
  FOR ii = 0L, n_valid-1L DO BEGIN
    ...
    ; Interpolate volume emissivity for all lines at this (Te, ne)
    ; Units: photons s^-1 cm^-3
    vol_emiss = IPT_INTERPOLATE_EMISSIVITY_2D_VECTORIZED(Te_los[i], ne_los[i], $
      tables_single.temp_arr, tables_single.dens_arr, emiss_table_species)

    ; Convert to per-ion photon emission rate: photons s^-1 ion^-1
    ; vol_emiss [photons/s/cm^3] / n_e [cm^-3] = per_ion_rate [photons/s/ion]
    ; But tables are already normalized by n_e in the CHIANTI calculation,
    ; so vol_emiss is actually in photons/s/cm^3 and needs to be divided by n_e
    ; to get the rate per ion. However, CHIANTI emiss_calc returns photons/s/cm^3
    ; which already includes the n_e factor. To get per-ion rate:
    ; per_ion_rate = vol_emiss / n_e
    ; Then brightness = per_ion_rate * n_ion

    ; Calculate per-ion photon emission rate
    per_ion_rate = vol_emiss / ne_los[i]  ; [photons s^-1 ion^-1]

    ; Brightness integrand: per_ion_rate * n_ion
    ; Units: [photons s^-1 ion^-1] * [ions cm^-3] = [photons s^-1 cm^-3]
...
FUNCTION IPT_INTEGRATE_SPECIES_EMISSION_DOUBLE, plasma_model, tables_single, $
  ...
  ;
  ; NOTES:
  ;   Falls back to single Maxwellian for points where feh < 0.001 or
  ;   ne_total < 1.0 cm^-3.
  ;-
  COMPILE_OPT IDL2, HIDDEN
  ...
  use_single = (feh_los LT 0.001) OR (ne_total_los LT 1.0)
  ...
  IF n_double GT 0 THEN BEGIN
    FOR ii = 0L, n_double-1L DO BEGIN
      ...
      ; Interpolate volume emissivity from 4D tables
      vol_emiss = IPT_INTERPOLATE_EMISSIVITY_4D_VECTORIZED(Tec_los[i], Teh_los[i], $
      ...
      ; Per-ion photon emission rate
      per_ion_rate = vol_emiss / ne_total_los[i]
  ...
  IF n_single GT 0 THEN BEGIN
    FOR ii = 0L, n_single-1L DO BEGIN
      ...
      ; Interpolate volume emissivity from 2D tables
      vol_emiss = IPT_INTERPOLATE_EMISSIVITY_2D_VECTORIZED(Tec_los[i], nec_los[i], $
        tables_single.temp_arr, tables_single.dens_arr, emiss_table_2d)

      ; Per-ion photon emission rate
      per_ion_rate = vol_emiss / nec_los[i]
...
PRO IPT_emiss_MOP_community_code
  ...
  PRINT, 'MOP Community Code v2.0 - Proper Line-of-Sight Integration'
  PRINT, '=========================================================================='
  PRINT, ''

...
# -*- coding: utf-8 -*-
"""
basic_example_uv_cm3_emission_model_use_tables.py
...
UV emission spectra from the Io Plasma Torus. It shows typical usage for both single
and double Maxwellian electron distributions and includes proper parameter choices
for realistic IPT conditions.
...
from IPT_cm3_emission_model import (
    ...
    analyze_key_uv_emission_lines
)


def main():
    """
    Example demonstrating UV emission modeling for Io Plasma Torus.
    
    This function shows typical usage of the emission table infrastructure for
    calculating IPT UV spectra. It includes both single and double Maxwellian
    cases and demonstrates proper parameter choices.
    ...
    results : dict
        ...
        - 'line_analysis': results from emission line analysis
    ...
    - Core temperature: 5 eV (from UV line ratios)
    ...
    # Europa-UVS and JUICE-UVS wavelength range and bin width
    min_xwav = 550.0    # Minimum wavelength [Angstroms]
    max_xwav = 2100.0   # Maximum wavelength [Angstroms]
    xwav_bin_width = 1.0  # Wavelength bin width [Angstroms]
    ...
    fwhm = 6.0  # FWHM [Angstroms] - Best case scenario for IPT observations from JUICE/Europa-UVS
    ...
    emiss_dir = Path(__file__).parent.parent.parent / 'Emiss_tables'
    ...
    tables.load_double_maxwellian_tables(
        str(emiss_dir / 'CHIANTI_11.0.2_emiss_tables_double_maxwellian_16x8x12x10.h5')
    )
    ...
    axes[0].set_title(
        f'IPT UV Emission: Single Maxwellian (Te={Tec:.1f} eV, ne={nec:.0f} cm⁻³)', 
        fontsize=14
    )
    ...
    axes[1].set_title(
        f'IPT UV Emission: Double Maxwellian (Tec={Tec:.1f} eV, Teh={Teh:.0f} eV, feh={feh:.4f})', 
        fontsize=14
    )
    ...
    plt.tight_layout()
    plt.savefig('ipt_uv_emission_single_vs_double.png', dpi=300)
    ...
    plt.tight_layout()
    plt.savefig('ipt_uv_emission_comparison.png', dpi=300)
    print("  Saved: ipt_uv_emission_comparison.png")
    
    plt.show()
    
    # ================================================================
    # ANALYZE SPECIFIC EMISSION LINES
    # ================================================================
    # Perform detailed analysis of key diagnostic emission lines
    # and generate high-resolution plots for scientific interpretation
    line_analysis_results = analyze_key_uv_emission_lines(
        xwav,
...
if __name__ == "__main__":
    ...
    results = main()
    # Extract results xwav= results['xwav']
    # spectrum = results['ypts_single_maxwellian'] etc...

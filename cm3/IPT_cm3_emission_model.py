...
def interpolate_emissivity_2d(tables: EmissionTables, 
    ...
    Uses bilinear interpolation in log10(T) - log10(n) space for accurate
    interpolation of emissivities which vary as power laws in temperature
    and density. The emission tables are sampled on a regular grid in 
    log-space, but the emissivity values themselves are linear (not logged).
    ...
    Physics Notes
    -------------
    Volume emissivity scaling:
    - ε ∝ n_e n_ion (collisional excitation rate depends on both densities)
    - ε ∝ exp(-ΔE/kT) (Boltzmann excitation factor)
    - ε ∝ T^(-1/2) (collision rate coefficient velocity dependence)
    
    Bilinear interpolation in log-space accurately captures these power-law 
    dependencies across orders of magnitude variation in plasma parameters.
    
    Implementation:
    - Locates surrounding grid cell in (log10(T), log10(n)) space
    - Computes normalized interpolation weights
    - Applies vectorized bilinear formula across all emission lines
    - Handles extrapolation by using edge values with warnings
    ...
    # Convert query point to log space
    log_T = np.log10(temperature)
    log_n = np.log10(density)
    
    # Get grid arrays in log space
    log_T_grid = tables.log_temp  # 1D array of log10(temperature)
    log_n_grid = tables.log_dens  # 1D array of log10(density)
    ...
    # searchsorted finds the insertion point to maintain sorted order
    i_T = np.searchsorted(log_T_grid, log_T)
    i_T = np.clip(i_T, 1, len(log_T_grid) - 1)  # Ensure valid bracketing indices
    ...
    i_n = np.clip(i_n, 1, len(log_n_grid) - 1)  # Ensure valid bracketing indices
    ...
    # w_T = 0 means at T0, w_T = 1 means at T1
    if log_T1 != log_T0:
        w_T = (log_T - log_T0) / (log_T1 - log_T0)
    else:
        w_T = 0.0  # Single grid point (shouldn't happen with valid tables)
    
    # w_n = 0 means at n0, w_n = 1 means at n1
    if log_n1 != log_n0:
        w_n = (log_n - log_n0) / (log_n1 - log_n0)
    else:
        w_n = 0.0  # Single grid point (shouldn't happen with valid tables)
    ...
    # Process each species separately to maintain proper line-species associations
    for species_key in species_list:
        ...
        # Get wavelengths and emission table for this species
        ...
        # Using vectorized indexing to process all lines simultaneously
        E_00 = emiss_table[:, i_T0, i_n0]  # Shape: (n_lines,) - low T, low n
        E_01 = emiss_table[:, i_T0, i_n1]  # Shape: (n_lines,) - low T, high n
        E_10 = emiss_table[:, i_T1, i_n0]  # Shape: (n_lines,) - high T, low n
        E_11 = emiss_table[:, i_T1, i_n1]  # Shape: (n_lines,) - high T, high n
        
        # Apply bilinear interpolation formula
        # E(T,n) = (1-w_T)(1-w_n)E_00 + (1-w_T)w_n*E_01 + w_T(1-w_n)E_10 + w_T*w_n*E_11
        # This is equivalent to: first interpolate in T at both n values,
        # then interpolate the results in n
        emiss_interpolated = (
            (1.0 - w_T) * (1.0 - w_n) * E_00 +
            (1.0 - w_T) * w_n * E_01 +
            w_T * (1.0 - w_n) * E_10 +
            w_T * w_n * E_11
        )
...
def interpolate_emissivity_4d(tables: EmissionTables, 
    ...
    Uses quadrilinear (4D linear) interpolation in the 4D parameter space
    (log10(Tec), log10(Teh), log10(ne), log10(feh)) to properly capture 
    nonlinear enhancement from hot electron population.
    ...
    tables : EmissionTables
        ...
        - emissivities_double: dict mapping species to emission rate arrays
                               with shape (n_lines, n_Tec, n_Teh, n_ne, n_feh)
    core_temp : float
    ...
    Physics Notes
    -------------
    The double Maxwellian distribution causes nonlinear enhancement of
    high-excitation lines. This cannot be approximated by weighted sum
    of two single Maxwellians because hot electrons have much larger 
    collision cross sections at high energies.
    
    Implementation
    --------------
    Fully vectorized quadrilinear interpolation using pre-computed weights
    applied to all 16 hypercube corners simultaneously for all emission lines.
    """
    ...
    # Pre-compute weight products for all 16 corners (2^4 = 16)
    # Binary indexing: bit 0 = Tec, bit 1 = Teh, bit 2 = ne, bit 3 = feh
    w0_Tec, w1_Tec = 1.0 - w_Tec, w_Tec
    w0_Teh, w1_Teh = 1.0 - w_Teh, w_Teh
    w0_ne, w1_ne = 1.0 - w_ne, w_ne
    w0_feh, w1_feh = 1.0 - w_feh, w_feh
    
    # All 16 corner weights as array for vectorized operations
    ...
    # Index arrays for the 16 corners
    ...
    for species_key in species_list:
        ...
        # Result shape: (16, n_lines)
        corners = emiss_table[:, idx_Tec, idx_Teh, idx_ne, idx_feh].T
        
        # Weighted sum across all corners: (16,) dot (16, n_lines) -> (n_lines,)
        emiss_interpolated = np.dot(corner_weights, corners)
